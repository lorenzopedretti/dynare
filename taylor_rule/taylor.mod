...
var C i pi N w m mc A Y vp pireset x1 x2 r;
varexo eA ei;
parameters sigma beta chi psi theta phi epsilon rhoA seA rhoi sei phipi PI;
sigma=1;
load param_phi;
...
A=rhoA*A(-1)+eA;
[name='Taylor Rule']
i=(1-rhoi)*STEADY_STATE(i)+rhoi*i(-1)+(1-rhoi)*(phipi*(pi-PI))+ei;
[name='Fisher']
1+i=(1+pi(+1))*(1+r);
end;
shocks;
